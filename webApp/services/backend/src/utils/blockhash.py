...
#
# Adapted liberally from https://github.com/commonsmachinery/blockhash-python
...
def hash_image(filename: str, resize_size):
    im = Image.open(filename)
    im = im.convert('RGB')

    if resize_size:
        size = resize_size.split('x')
        size = (int(size[0]), int(size[1]))
        im = im.resize(size, Image.NEAREST)

    image_hash = blockhash(im, 16)

    folder = os.path.dirname(filename)
    im.save(os.path.join(folder, image_hash + ".jpg"), "JPEG")
